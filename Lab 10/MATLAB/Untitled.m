...
line_code_temp = serial_bin * 2 - 1 ;
line_code = [];
for i  = 1 : length(serial_bin)
    for j = 1 : RES
        line_code = [line_code line_code_temp(i)];
    end
end

t_vec = 1/(RES * BITRATE):1/(RES * BITRATE) : length(serial_bin)/BITRATE;
...
for i=serial_bin
    ...
    %f_t = rectpuls(t - t_mid, tpb)%%%%%%%%%%%%%%%%%%%%%%%%%%

    if MF_CONV_SAME
        y_mf_t = conv(y_t, f_t, 'same');
        t_mf = t;
    else
        y_mf_t = conv(y_t, f_t, 'full');%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
        if REMOVE_OVERLAP%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
